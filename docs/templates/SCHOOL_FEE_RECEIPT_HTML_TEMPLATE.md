# School fee receipt (HTML template)

Institute-specific template for type `SCHOOL_FEE_RECEIPT`. Placeholders are replaced by `SchoolFeeReceiptService` (`{{fee_table}}` is injected server-side). Use `{{TOTAL_CONCESSION_ROW_STYLE}}` on the Total Concession row: backend sets `display: none;` when total concession is zero.
  ...
  <style>
    ...
    */
    ...
    /* status cells from backend:
       ...
       — printable: no fill colours, same ink as body text
    */
    td.status-PAID,
    td.status-PARTIAL_PAID,
    td.status-PENDING {
      color: #000;
      font-weight: bold;
...
<body>
  <div class="page">
    ...
    <!-- ══════════════════════════════
         FEE TABLE
         backend injects a full <table>...</table> into {{fee_table}}
         Backend: use header "Concession" (not "Discount") and omit concession
         column/cells when concession is 0 — same data source as UI discount field.
    ══════════════════════════════ -->
    <div class="table-section">
      ...
      <div class="totals-section">
        <table class="totals-table">
          ...
          <tr style="{{TOTAL_CONCESSION_ROW_STYLE}}">
