...
public interface StudentSessionRepository extends CrudRepository<StudentSessionInstituteGroupMapping, String> {
        ...
        List<StudentSessionInstituteGroupMapping> findAllByUserPlanIdAndStatusIn(String userPlanId,
                        List<String> status);
         ...
         * change.
         * This marks previous ONLY_DETAILS_FILLED and PAYMENT_FAILED entries as
         * DELETED.
         */
        @Modifying
        @Transactional
         ...
         * Used by workflow to find ONLY_DETAILS_FILLED entries.
         */
        @Query("SELECT s FROM StudentSessionInstituteGroupMapping s " +
                        "WHERE s.userId = :userId " +
                        "AND s.packageSession.id = :packageSessionId " +
        ...
        @Query("SELECT s FROM StudentSessionInstituteGroupMapping s " +
                        "WHERE s.userId = :userId " +
                        "AND s.destinationPackageSession.id IN :destinationPackageSessionIds " +
                        "AND s.institute.id = :instituteId " +
                        "AND s.type IN :types " +
                        "AND s.status IN :statuses")
        List<StudentSessionInstituteGroupMapping> findByUserAndDestinationsAndTypeAndStatus(
                        @Param("userId") String userId,
