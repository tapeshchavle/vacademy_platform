...
public interface InstituteStudentRepository extends CrudRepository<Student, String> {
    ...
    @Query(nativeQuery = true, value = """
        SELECT
            ...
            CAST(
              COALESCE(
                ...
                ) FILTER (WHERE cfv.value IS NOT NULL), '[]'
              ) AS text
            ) AS "customFieldsJson",
            s.user_id AS "userId",
            s.id AS "id",
        ...
        LEFT JOIN custom_fields cf
            ON cf.id = icf.custom_field_id
        LEFT JOIN LATERAL (
            SELECT cfv_inner.value
            FROM custom_field_values cfv_inner
            WHERE cfv_inner.source_type = 'STUDENT_SESSION_INSTITUTE_GROUP_MAPPING'
              AND cfv_inner.source_id = ssigm.id
              AND cfv_inner.custom_field_id = cf.id
              AND cfv_inner.value IS NOT NULL
            ORDER BY
              cfv_inner.created_at DESC
            LIMIT 1
        ) cfv ON TRUE
        LEFT JOIN user_plan up
            ON up.id = ssigm.user_plan_id
    ...
    @Query(nativeQuery = true, value = """
        SELECT
            ...
            CAST(
              COALESCE(
                ...
                -- MODIFICATION 1: Only aggregate where a non-null value was found
                ) FILTER (WHERE cfv.value IS NOT NULL), '[]'
              ) AS text
            ) AS "customFieldsJson",
            s.user_id AS "userId",
        ...
        LEFT JOIN institute_custom_fields icf
            ON icf.institute_id = ssigm.institute_id
            -- MODIFICATION 2: Updated SpEL expression for consistency
            AND (:#{#customFieldStatus == null || #customFieldStatus.isEmpty()} = true OR icf.status IN (:customFieldStatus))
        LEFT JOIN custom_fields cf
            ON cf.id = icf.custom_field_id

        -- MODIFICATION 3: Use LEFT JOIN LATERAL to get ONLY the latest non-null value
        LEFT JOIN LATERAL (
            SELECT cfv_inner.value
            FROM custom_field_values cfv_inner
            WHERE cfv_inner.source_type = 'STUDENT_SESSION_INSTITUTE_GROUP_MAPPING'
