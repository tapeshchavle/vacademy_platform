...
public class InstituteInitManager {
     ...
     * @param includePrivateFields Whether to include private fields (email, phone, subjects, etc.)
    ...
    private InstituteInfoDTO buildInstituteInfoDTO(String instituteId, boolean includePrivateFields) {
        ...
        List<PackageSession> packageSessions = 
                packageSessionRepository.findPackageSessionsByInstituteId(instId, activeStatuses);
        ...
        dto.setBatchesForSessions(packageSessions.stream()
                .map(obj -> new PackageSessionDTO(obj, readTimeMap.getOrDefault(obj.getId(), 0.0)))
                .toList());
