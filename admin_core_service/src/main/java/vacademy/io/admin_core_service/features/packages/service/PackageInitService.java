...
public class PackageInitService {
    ...
    public PackageFilterDetailsDTO getPackageFilterDetails(List<String> packageSessionIds) {
       ...
       packageFilterDetailsDTO.setBatchesForSession(packageSessionRepository.findPackageSessionsByIds(packageSessionIds).stream().map((obj)->new PackageSessionDTO(obj,slideService.calculateTotalReadTimeInMinutes(obj.getId()))).toList());
