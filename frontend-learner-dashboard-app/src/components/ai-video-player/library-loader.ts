...
/**
 * Get library URLs for a content type
 */
...
/**
 * Get script tags for libraries
 */
export function getLibraryScriptTags(contentType: ContentType): string {
  const urls = getLibraryUrls(contentType);
  const styles = CSS_CONFIG[contentType] || [];

  const styleTags = styles.map(url => `<link rel="stylesheet" href="${url}">`).join('\n');
  const scriptTags = urls.map(url => `<script src="${url}"></script>`).join('\n');
