...
export function useServerTime() {
  ...
  const fetchServerTime = useCallback(async () => {
    ...
    try {
      ...
      const response = await axios.get<ServerTimeResponse>(GET_SERVER_TIME);
      const end = Date.now();
      ...
      setError(
        err instanceof Error ? err : new Error("Failed to fetch server time")
      );
