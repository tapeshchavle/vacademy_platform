...
import { AudioTrackingStore, AudioTrackingDataSchema, AudioActivitySchema } from '@/schemas/study-library/audio-tracking-schema';
import { z } from 'zod';

const STORAGE_KEY = 'audio_tracking_data';

// Function to load data from storage
const loadFromStorage = async (): Promise<z.infer<typeof AudioTrackingDataSchema>> => {
    try {
...
export const useAudioTrackingStore = create<AudioTrackingStore>((set) => ({
    trackingData: { data: [] },
    ...
    addActivity: async (activity, isUpdate = false) => {
        try {
            ...
            const existingActivities = storedData.data as Array<z.infer<typeof AudioActivitySchema>>;
    ...
    getStoredActivities: async () => {
        try {
            const storedData = await loadFromStorage();
            console.log('Loaded stored audio activities:', storedData);
            set({ trackingData: storedData });
        } catch (error) {
            console.error('Failed to get stored audio activities:', error);
            throw error;
        }
    }
}));
