...
const TodoList: React.FC = () => {
  const [todos, setTodos] = useState<Todo[]>(() => TODO_ITEMS.map(t => ({ ...t, completed: false })));
  ...
  const handleItemComplete = (todo: Todo) => {
    ...
    const payload = SLIDE_PAYLOADS[todo.path] || { content: `Placeholder content for ${todo.text}`, slideType };
      ...
      <ul className="todo-list">
        {todos.slice(0, activeIndex + 1).map((todo, idx) => (
          <TodoItem
            ...
            onComplete={idx === activeIndex ? () => handleItemComplete(todo) : undefined}
          />
        ))}
      </ul>
    </div>
