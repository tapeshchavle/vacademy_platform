        ...
        <div className="flex h-full w-[307px] flex-col gap-4 bg-primary-50 px-3 py-6">
            ...
            <div className="flex-1 space-y-1 overflow-y-auto pr-1">
                {sideBarList.map((course) => {
                    ...
                    const levels = sessions.flatMap((session) => getCourseLevels(course.id, session.id));
                    ...
                    return (
                        <Collapsible key={course.id} open={course.id === courseId} className="space-y-1">
                            <CollapsibleTrigger
                                className="w-full"
                            ...
                            >
                                <div
                                    className={cn(
                                        ...
                                        course.id === courseId ? 'bg-primary-100' : 'hover:bg-gray-200'
                                    )}
                                >
                                    <Folder
                                        size={18}
                                    ...
                                    />
                                    <span className="flex-1 truncate text-left">{course.value}</span>
                                    <CaretRight
                                        size={14}
                                        weight="bold"
                                        className={cn(
                                        ...
                                        >
                                            ...
                                            >
                                                ...
                                                >
                                                    <Book
                                                        size={16}
                                                        weight={level.id === levelId ? 'fill' : 'duotone'}
                                                        className={cn(level.id === levelId && 'text-primary-600')}
                                                    />
                                                ...
                                                {subjects.map((subject) => (
                                                    ...
                                                    >
                                                        <Bookmark
                                                            size={14}
                                                            weight={
                                                                subject.id === subjectId ? 'fill' : 'duotone'
                                                            }
                                                        />
                                                        <span className="flex-1 truncate text-left">
