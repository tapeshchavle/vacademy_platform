...
export function LoginForm({
            ...
            if (authorityKeys.length > 1) {
                ...
                if (instituteId && userId) {
                    try {
                        ...
                        setPrimaryColor(
                            details?.institute_theme_code ?? "#E67E22"
                        );
                    } catch (error) {
                        console.error(
    ...
    return variant === "dialog" ? (
        <div
            className={`${variant === "dialog" ? "flex items-center justify-center h-auto" : "w-screen  gap-4 md:gap-8 lg:gap-10 pt-14 lg:pt-20"}`}
        >
            <div
                className={`${variant === "dialog" ? "w-[40vw] max-w-sm h-auto rounded-xl  p-1 gap-2 flex flex-col" : "flex w-full flex-col items-center justify-center gap-4 md:gap-8 lg:gap-12 px-4 md:px-8 lg:px-12"}`}
            >
                ...
                {/* OAuth Login Buttons */}
                <div
                    className={`${variant === "dialog" ? "flex flex-col w-full items-center gap-3" : "flex w-full flex-col items-center justify-center gap-4 px-4 md:px-8 lg:px-12"}`}
                >
                    <button
                    ...
                    <div
                        className={`${variant === "dialog" ? "relative w-full flex items-center justify-center mt-2 mb-1" : "relative flex items-center justify-center"}`}
                    >
                        <div className="absolute inset-0 flex items-center">
                            <span className="w-full border-t border-neutral-300" />
                        </div>
                        <div
                            className={`${variant === "dialog" ? "px-2 text-xs" : "px-4 text-sm"} relative bg-white  text-neutral-500`}
                        >
                            or continue with
                        </div>
                    </div>
                ...
                <div
                    className={`${variant === "dialog" ? "" : "max-w-md"}w-full`}
                >
                    {isEmailLogin ? (
                        <EmailLogin
