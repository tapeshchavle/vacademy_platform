...
import { fetchExcalidrawContent } from './excalidrawUtils';
import type {
...
> {
  ...
  componentDidCatch(error: Error, errorInfo: React.ErrorInfo) {
    console.error('ExcalidrawErrorBoundary caught an error:', error, errorInfo);
...
// Explicitly type the lazy loaded component with error boundary and React 19 compatibility
const Excalidraw = lazy(() => 
  import('@excalidraw/excalidraw').then(module => {
    // Ensure we're using the same React context and handle React 19 compatibility
    return { default: module.Excalidraw as ComponentType<ExcalidrawProps> };
  }).catch(error => {
    ...
    return (
      <div className="flex items-center justify-center h-full">
        <div className="text-center">
          ...
          <p className="text-gray-600 text-sm">Loading drawing...</p>
        </div>
      </div>
    );
  }
      ...
      <div className="w-full h-full excalidraw-wrapper bg-slate-50 rounded-lg overflow-hidden shadow-sm relative">
        ...
        <Suspense fallback={
          <div className="flex items-center justify-center h-full">
            <div className="text-center">
              <div className="animate-spin rounded-full h-8 w-8 border-b-2 border-primary-500 mx-auto mb-2"></div>
              <p className="text-gray-600 text-sm">Initializing Excalidraw...</p>
            </div>
          </div>
        }>
          <ExcalidrawErrorBoundary onError={(error) => setError(`React version compatibility issue: ${error.message}`)}>
            <Excalidraw
              key={fileId}
              excalidrawAPI={(api) => {
