    ...
    const out: StudentDisplaySettingsData = {
        sidebar: {
            ...
            tabs: mergeArrayById<StudentSidebarTabConfig>(
                ...
                id: t.id,
                label:
                    t.label ?? d.sidebar.tabs.find((x) => x.id === t.id)?.label,
                route:
                    t.route ?? d.sidebar.tabs.find((x) => x.id === t.id)?.route,
                order:
                    t.order ??
                    d.sidebar.tabs.find((x) => x.id === t.id)?.order ??
                    0,
                visible:
                    t.visible ??
                ...
                subTabs: mergeArrayById(
                    incoming?.sidebar?.tabs?.find((x) => x?.id === t.id)
                        ?.subTabs,
        ...
        dashboard: {
            widgets: mergeArrayById<StudentDashboardWidgetConfig>(
                ...
                order:
                    w.order ??
                    d.dashboard.widgets.find((x) => x.id === w.id)?.order ??
                    0,
                visible:
                    w.visible ??
                ...
                title:
                    w.title ??
                    d.dashboard.widgets.find((x) => x.id === w.id)?.title,
                subTitle:
                    w.subTitle ??
                    d.dashboard.widgets.find((x) => x.id === w.id)?.subTitle,
                link:
                    w.link ??
                    d.dashboard.widgets.find((x) => x.id === w.id)?.link,
            })),
        },
        signup: {
            providers: {
                google:
                    incoming?.signup?.providers?.google ??
                    d.signup.providers.google,
                github:
                    incoming?.signup?.providers?.github ??
                    d.signup.providers.github,
                usernamePassword:
                    incoming?.signup?.providers?.usernamePassword ??
                    d.signup.providers.usernamePassword,
                emailOtp:
                    incoming?.signup?.providers?.emailOtp ??
                    d.signup.providers.emailOtp,
                defaultProvider:
        ...
        permissions: {
            canViewProfile:
                incoming?.permissions?.canViewProfile ??
                d.permissions.canViewProfile,
            canEditProfile:
                incoming?.permissions?.canEditProfile ??
                d.permissions.canEditProfile,
            canDeleteProfile:
        ...
        courseDetails: {
            tabs: mergeArrayById(
                ...
                label:
                    t.label ??
                    d.courseDetails.tabs.find((x) => x.id === t.id)?.label,
                order:
                    t.order ??
            ...
            defaultTab:
                incoming?.courseDetails?.defaultTab ??
                d.courseDetails.defaultTab,
            outlineMode:
                incoming?.courseDetails?.outlineMode ??
                d.courseDetails.outlineMode,
        ...
        allCourses: {
            tabs: mergeArrayById(
                incoming?.allCourses?.tabs,
                d.allCourses.tabs
            ).map((t) => ({
                id: t.id,
                label:
                    t.label ??
                    d.allCourses.tabs.find((x) => x.id === t.id)?.label,
                order:
                    t.order ??
                    d.allCourses.tabs.find((x) => x.id === t.id)?.order ??
                    0,
                visible:
                    t.visible ??
            ...
            defaultTab:
                incoming?.allCourses?.defaultTab ?? d.allCourses.defaultTab,
        },
        notifications: {
            allowSystemAlerts:
