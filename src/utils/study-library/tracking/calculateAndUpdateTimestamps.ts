...
export const calculateAndUpdateTimestamps = (
    activity: z.infer<typeof ActivitySchema>
) => {
    const totalTimestampDuration = activity.timestamps.reduce(
        (
            sum: number,
            timestamp: {
                start: number;
    ...
    if (
        activityDuration > totalTimestampDuration &&
        activity.current_start_time
    ) {
        const remainingDuration = activityDuration - totalTimestampDuration;
        const endTimeInEpoch: number =
            activity.current_start_time_in_epoch + remainingDuration;

        const newTimestamp = {
            id: uuidv4(),
            start_time: formatVideoTime(
