...
const openTestLoginSchema = z.object({
  email: z.string().min(1, "Email is required").email("Invalid email format"),
  password: z.string().min(6, "Password must be at least 6 characters"),
});

type FormValues = z.infer<typeof openTestLoginSchema>;
function RouteComponent() {
  const navigate = useNavigate();
  const form = useForm<FormValues>({
    resolver: zodResolver(openTestLoginSchema),
    defaultValues: {
      email: "",
      password: "",
    },
        ...
        <div className="flex justify-center items-start w-1/2 flex-col">
          <FormProvider {...form}>
            <form className="w-full flex flex-col gap-6 mt-4">
              ...
              />
              <h1
                className="text-primary-500 text-sm -mt-4 font-thin cursor-pointer"
                onClick={() =>
                  navigate({
                    to: "/register/login/forgot-password",
                  })
                }
              >
                Forgot Password?
