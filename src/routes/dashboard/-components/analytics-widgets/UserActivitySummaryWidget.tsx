...
export default function UserActivitySummaryWidget({ instituteId }: UserActivitySummaryWidgetProps) {
  ...
  return (
    <motion.div
      ...
      className="h-full"
    >
      <Card className="border-0 shadow-lg hover:shadow-xl transition-all duration-300 bg-gradient-to-br from-white to-neutral-50 h-full">
        <CardHeader className="pb-3">
          <div className="flex items-center gap-2">
            <motion.div
              animate={{ rotate: [0, 10, -10, 0] }}
              transition={{ duration: 2, repeat: Infinity, delay: 1 }}
              className="p-2 rounded-lg bg-gradient-to-r from-primary-500 to-primary-600 text-white"
            >
              <Activity size={20} />
            </motion.div>
            <div>
            ...
            <div className="grid grid-cols-1 gap-2 sm:grid-cols-2 sm:gap-3">
              {stats.map((stat, index) => (
                <motion.div
                  ...
                  className={`p-3 rounded-lg border ${stat.bgColor} ${stat.borderColor} hover:shadow-md transition-all duration-200`}
                >
