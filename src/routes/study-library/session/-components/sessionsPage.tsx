    ...
    return (
        <div>
            ...
            <div className="my-10 flex flex-col gap-6 text-neutral-600">
                {sessionData?.length ? sessionData?.map((session, idx) => <SessionCard key={idx} data={session} />) : (
                    <div className="flex flex-col items-center justify-center gap-2 h-[50vh]"> <EmptySessionImage /> <p className="text-body text-neutral-600">No sessions found</p></div>
                )}
            </div>
            <NoCourseDialog isOpen={isOpen} setIsOpen={setIsOpen} type="Adding Sessions" content="You need to create a course before" />
        </div>
    );
}
