...
export const fetchCompleteInstituteDetails = async () => {
  const instituteId = await getInstituteId();
  const userId = await getUserId();
  const response = await authenticatedAxiosInstance({
    method: "GET",
    url: `${INSTITUTE_COMPLETE_DETAILS}/${instituteId}`,
    params: {
      instituteId,
      userId,
    },
...
export const handleFetchCompleteInstituteDetails = () => {
  return {
    queryKey: ["FETCH_INSTITUTE_COMPLETE_DETAILS"],
    queryFn: () => fetchCompleteInstituteDetails(),
    staleTime: 60 * 60 * 1000,
  };
};
...
export const fetchUserRolesDetails = async () => {
  try {
    ...
    if (!userId) {
      throw new Error(
        "Could not determine userId from token or StudentDetails",
      );
    }
