...
export const SendEmailDialog = () => {
    const { isSendEmailOpen, bulkActionInfo, selectedStudent, isBulkAction, closeAllDialogs } = useDialogStore();
    ...
    const [customMessage, setCustomMessage] = useState('Thank you for being part of our learning community.');
    ...
    const handleSendBulkEmail = async () => {
        ...
        const apiUsersPayload = studentEmailStatuses.map(statusEntry => {
        ...
        try {
            ...
            if (response.ok) {
                toast.success(`Successfully sent request for ${apiUsersPayload.length} email(s).`, { id: 'bulk-email-progress' });
                setStudentEmailStatuses(prevStatuses => 
    ...
    useEffect(() => {
        if (isSendEmailOpen) {
            ...
            setStudentEmailStatuses(studentsWithEmail.map(student => ({
            ...
            footer={
                <div className="flex items-center justify-end gap-2">
                    ...
                    <MyButton
                        ...
                        disable={!emailSubject.trim() || !emailBody.trim() || recipientCount === 0 || isBulkEmailSending}
        ...
        >
            <div className="space-y-4">
                <div className="text-sm text-neutral-600 mb-4">
                    Compose your email below. {recipientCount} student(s) with email addresses will receive it.
                </div>
                    ...
                    <div className="mb-3 flex flex-wrap gap-1">
                        {PLACEHOLDER_VARIABLES.map(placeholder => (
                            <MyButton
                                ...
                                disabled={isBulkEmailSending}
                                className="text-xs px-2 py-1 hover:bg-blue-50 hover:text-blue-700"
                            >
                                {placeholder.label}
                            </MyButton>
                        ))}
                ...
                {isBulkEmailSending && studentEmailStatuses.length > 0 && (
                    <div className="max-h-48 overflow-y-auto space-y-2 pr-2">
                        <p className="text-sm font-medium mb-2">
                            Sending Progress ({studentEmailStatuses.filter(s => s.status === 'sent' || s.status === 'failed').length}/{studentEmailStatuses.length}):
                        </p>
                        {studentEmailStatuses.map(s => (
                            <div key={s.userId} className="text-xs flex justify-between items-center p-1.5 rounded bg-neutral-100">
                                <span className="truncate max-w-[200px]">{s.name} ({s.email})</span>
            ...
            <div className="space-y-4">
                ...
                {generatePreview().subject && (
                    <div className="space-y-2">
                        ...
                        <div className="p-3 bg-neutral-50 border border-neutral-200 rounded-lg text-neutral-800">
                            {generatePreview().subject}
                        </div>
                    </div>
                )}
                ...
                {generatePreview().body && (
                    <div className="space-y-2">
                        ...
                        <div className="p-4 bg-neutral-50 border border-neutral-200 rounded-lg text-neutral-800 whitespace-pre-wrap max-h-96 overflow-y-auto">
                            {generatePreview().body}
                        </div>
                    </div>
                )}
