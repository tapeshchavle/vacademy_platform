...
export const duplicateKeyCheck = (customFields: CustomField[], currentFieldName: string) => {
    ...
    const duplicateKey = customFields.some((field) => getCustomFieldKey(field.name) === currentFieldKey);
    if(currentFieldKey === 'gender' || currentFieldKey === 'state' || currentFieldKey === 'city' || currentFieldKey === 'school_college' || currentFieldKey === 'address' || currentFieldKey === 'pincode' || currentFieldKey === 'father_name' || currentFieldKey === 'mother_name' || currentFieldKey === 'parent_phone_number' || currentFieldKey === 'parent_email') {
        return true;
    }
    return duplicateKey;
}
