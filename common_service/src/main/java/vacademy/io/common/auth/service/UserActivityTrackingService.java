...
public class UserActivityTrackingService {
    ...
    public UserActivityAnalyticsDto getInstituteAnalytics(String instituteId) {
        ...
        List<UserActivityAnalyticsDto.ServiceUsageDto> serviceUsage = getServiceUsageWithUserDetails(instituteId,
                now.minusDays(7));
        ...
        List<UserActivityAnalyticsDto.HourlyActivityDto> hourlyActivity = getHourlyActivityWithUserDetails(instituteId,
                now.minusDays(1));
                ...
                .map(session -> userRepository.findById(session.getUserId())
                        .map(user -> {
                            ...
                            Long sessionDuration = session.getLoginTime() != null
                                    ? java.time.Duration.between(session.getLoginTime(), LocalDateTime.now())
                                            .toMinutes()
                                    : 0L;
    ...
    private List<UserActivityAnalyticsDto.ServiceUsageDto> getServiceUsageWithUserDetails(String instituteId,
        ...
        return serviceStats.stream()
                .map(data -> {
                    ...
                    List<UserActivityAnalyticsDto.ServiceUserDto> topUsers = getTopUsersForService(instituteId,
    ...
    private List<UserActivityAnalyticsDto.DeviceUsageDto> getDeviceUsageWithUserDetails(String instituteId,
        ...
        return deviceStats.stream()
                .map(data -> {
                    ...
                    List<UserActivityAnalyticsDto.DeviceUserDto> topUsers = getTopUsersForDeviceType(instituteId,
                            deviceType, startTime, 5);

                    return UserActivityAnalyticsDto.DeviceUsageDto.builder()
                            .deviceType(deviceType)
                            .usageCount(topUsers.stream()
                                    .mapToLong(UserActivityAnalyticsDto.DeviceUserDto::getUsageCount).sum())
                ...
                .map(data -> {
                    ...
                    return userRepository.findById(userId)
                            .map(user -> {
                                ...
                                List<String> frequentServices = getUserFrequentServices(userId, instituteId,
                                        startDate.atStartOfDay());

                                // Get user's preferred device type
                                String preferredDeviceType = getUserPreferredDeviceType(userId, instituteId,
                                        startDate.atStartOfDay());
    ...
    private List<UserActivityAnalyticsDto.HourlyActivityDto> getHourlyActivityWithUserDetails(String instituteId,
        ...
        return hourlyStats.stream()
                .map(data -> {
                    ...
                    List<UserActivityAnalyticsDto.HourlyActiveUserDto> topUsers = getTopUsersForHour(instituteId, hour,
                            startTime, 3);
     ...
     */
    private List<UserActivityAnalyticsDto.ServiceUserDto> getTopUsersForService(String instituteId, String serviceName,
     ...
     */
    private List<UserActivityAnalyticsDto.DeviceUserDto> getTopUsersForDeviceType(String instituteId, String deviceType,
            LocalDateTime startTime, int limit) {
        List<Object[]> topUsersData = activityLogRepository.getMostActiveUsers(instituteId, startTime);
