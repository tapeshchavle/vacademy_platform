...
public interface DailyUserActivitySummaryRepository extends JpaRepository<DailyUserActivitySummary, String> {
    ...
    // Institute-wide analytics - Using native query (institute_id is VARCHAR, not UUID)
    @Query(value = "SELECT SUM(d.total_sessions), SUM(d.total_activity_time_minutes), SUM(d.total_api_calls) " +
           "FROM daily_user_activity_summary d WHERE d.institute_id = :instituteId AND d.activity_date = :date", 
           nativeQuery = true)
    Object[] getInstituteActivitySummaryForDate(@Param("instituteId") String instituteId, @Param("date") LocalDate date);

    @Query(value = "SELECT COUNT(DISTINCT d.user_id) FROM daily_user_activity_summary d WHERE d.institute_id = :instituteId AND d.activity_date = :date", 
           nativeQuery = true)
    Long countActiveUsersForDate(@Param("instituteId") String instituteId, @Param("date") LocalDate date);

    // Trending data - Using native query (institute_id is VARCHAR, not UUID)
    @Query(value = "SELECT d.activity_date, COUNT(DISTINCT d.user_id), SUM(d.total_sessions), SUM(d.total_api_calls) " +
           "FROM daily_user_activity_summary d WHERE d.institute_id = :instituteId AND d.activity_date BETWEEN :startDate AND :endDate " +
    ...
    // Most active users in date range - Using native query (institute_id is VARCHAR, not UUID)
    ...
    // Service usage trends - Using native query (institute_id is VARCHAR, not UUID)
    @Query(value = "SELECT d.services_used FROM daily_user_activity_summary d WHERE d.institute_id = :instituteId AND d.activity_date BETWEEN :startDate AND :endDate", 
           nativeQuery = true)
    ...
    // Device usage trends - Using native query (institute_id is VARCHAR, not UUID)
    @Query(value = "SELECT d.device_types_used FROM daily_user_activity_summary d WHERE d.institute_id = :instituteId AND d.activity_date BETWEEN :startDate AND :endDate", 
           nativeQuery = true)
    ...
    // Average session duration trends - Using native query (institute_id is VARCHAR, not UUID)
    ...
    // Peak activity hours - Using native query (institute_id is VARCHAR, not UUID)
    @Query(value = "SELECT d.peak_activity_hour, COUNT(d.*) FROM daily_user_activity_summary d WHERE d.institute_id = :instituteId AND d.activity_date BETWEEN :startDate AND :endDate " +
    ...
    // User engagement metrics - Using native query (institute_id is VARCHAR, not UUID)
    @Query(value = "SELECT COUNT(d.*) FROM daily_user_activity_summary d WHERE d.institute_id = :instituteId AND d.user_id = :userId AND d.activity_date BETWEEN :startDate AND :endDate", 
           nativeQuery = true)
